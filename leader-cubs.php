...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['create_task'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO tasks (title, description, deadline, leader_id, target_group, image, video_link, duration_minutes, grade_over, created_at) VALUES (?, ?, ?, ?, 'cubs', ?, ?, ?, ?, NOW())");
    $stmt->bind_param("sssissii", $title, $description, $deadline, $userId, $image_name, $video_link, $duration, $grade_over);
    if ($stmt->execute()) {
        $task_id = $stmt->insert_id;
        $success = "Task added successfully!";
        $cubs = $conn->query("SELECT id FROM individual WHERE TIMESTAMPDIFF(YEAR, date_of_birth, CURDATE()) BETWEEN 5 AND 9");
        while ($cub = $cubs->fetch_assoc()) {
          $conn->query("INSERT INTO grades (task_id, individual_id, grade) VALUES ($task_id, {$cub['id']}, NULL)");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['save_grades'])) {
    foreach ($_POST['grades'] as $grade_id => $grade) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['save_schedule'])) {
    ...
    $note = $_POST['note'];
    $stmt = $conn->prepare("INSERT INTO schedule (schedule_date, note) VALUES (?, ?)");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['save_attendance'])) {
  ...
  $conn->query("DELETE FROM attendance WHERE attend_date = '$attend_date'");

  foreach ($attendance as $id => $status) {
      $stmt = $conn->prepare("INSERT INTO attendance (individual_id, attend_date, status) VALUES (?, ?, ?)");
...
$tasks = $conn->query("SELECT id, title FROM tasks WHERE target_group = 'cubs'");
$cubs = $conn->query("SELECT id, CONCAT(first_name, ' ', last_name) AS name FROM individual WHERE TIMESTAMPDIFF(YEAR, date_of_birth, CURDATE()) BETWEEN 5 AND 9");
...
if ($tasks && $cubs) {
    ...
    while ($cub = $cubs->fetch_assoc()) {
        ...
        foreach ($task_list as $task_id => $task_title) {
            $grade_q = $conn->query("SELECT id, grade FROM grades WHERE task_id = $task_id AND individual_id = {$cub['id']}");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_task'])) {
  ...
  $stmt = $conn->prepare("UPDATE tasks SET title=?, description=?, deadline=?, video_link=?, duration_minutes=?, grade_over=? WHERE id=?");
...
$edit_tasks = $conn->query("SELECT * FROM tasks WHERE target_group = 'cubs' ORDER BY created_at DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
            ...
            <table class="table table-bordered">
              ...
              <?php foreach ($grades_data as $cub): ?>
                ...
                <?php foreach ($cub['grades'] as $grade): ?>
                  <td><input type="number" class="form-control" name="grades[<?= $grade['id'] ?>]" value="<?= $grade['grade'] ?>"></td>
                <?php endforeach; ?>
                </tr>
              <?php endforeach; ?>
              </tbody>
    ...
    <table class="table table-bordered">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            <select name="attendance[<?= $cub['id'] ?>]" class="form-control">
...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
      ...
      events: [
        <?php
        $sched = $conn->query("SELECT schedule_date, note FROM schedule");
