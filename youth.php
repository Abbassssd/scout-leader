...
if ($age < 15 || $age > 19) {
    echo "Access Denied. Only youth (15-19 years old) can access this page.";
    exit();
}

// Fetch tasks assigned for cubs
$tasks = [];
$task_query = $conn->query("SELECT * FROM tasks3 WHERE target_group = 'cubs' ORDER BY created_at DESC");
...
// Fetch news
$news_list = [];
$news_query = $conn->query("SELECT * FROM news3 ORDER BY created_at DESC");
  ...
  <div class="row">
    ...
    <div class="col-md-3 sidebar d-flex flex-column">
      ...
      <div class="logout-btn">
        <a href="index.html" class="btn btn-danger">Sign Out</a>
      </div>
    </div>
...
<?php endif; ?>
        ...
        <div class="tab-pane fade" id="grades" role="tabpanel">
          ...
          <table class="table table-bordered">
            ...
            <tbody>
              <?php
              $grades = $conn->query("SELECT t.title, g.grade, t.grade_over FROM grades3 g JOIN tasks3 t ON g.task_id = t.id WHERE g.individual_id = $userId");
  ...
  <table class="table table-bordered">
    ...
    <tbody>
      <?php
      $attendance = $conn->query("SELECT attend_date, status FROM attendance3 WHERE individual_id = $userId ORDER BY attend_date DESC");
