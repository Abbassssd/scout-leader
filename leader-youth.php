...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['create_news'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO news3 (title, content, image, created_at) VALUES (?, ?, ?, NOW())");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['create_task'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO tasks3 (title, description, deadline, leader_id, target_group, image, video_link, duration_minutes, grade_over, created_at) VALUES (?, ?, ?, ?, 'cubs', ?, ?, ?, ?, NOW())");
    $stmt->bind_param("sssissii", $title, $description, $deadline, $userId, $image_name, $video_link, $duration, $grade_over);
    if ($stmt->execute()) {
        $task_id = $stmt->insert_id;
        $success = "Task added successfully!";
        $cubs = $conn->query("SELECT id FROM individual WHERE TIMESTAMPDIFF(YEAR, date_of_birth, CURDATE()) BETWEEN 15 AND 19");
        while ($cub = $cubs->fetch_assoc()) {
            $conn->query("INSERT INTO grades3(task_id, individual_id, grade) VALUES ($task_id, {$cub['id']}, NULL)");
        }
    } else {
...
// Save Grades
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['save_grades']) && isset($_POST['grades3'])) {
    foreach ($_POST['grades3'] as $grade_id => $grade) {
        $grade = is_numeric($grade) ? $grade : "NULL";
        $grade_id = intval($grade_id);
      $conn->query("UPDATE grades3 SET grade = $grade WHERE id = $grade_id");
  }
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['save_attendance'])) {
  ...
  $attendance = $_POST['attendance3'] ?? [];

  // Delete old records for same date to prevent duplicates
  $conn->query("DELETE FROM attendance3 WHERE attend_date = '$attend_date'");

  foreach ($attendance as $id => $status) {
      $stmt = $conn->prepare("INSERT INTO attendance3 (individual_id, attend_date, status) VALUES (?, ?, ?)");
...
$tasks2 = $conn->query("SELECT id, title FROM tasks3 WHERE target_group = 'cubs'");
$cubs = $conn->query("SELECT id, CONCAT(first_name, ' ', last_name) AS name FROM individual WHERE TIMESTAMPDIFF(YEAR, date_of_birth, CURDATE()) BETWEEN 15 AND 19");
...
if ($tasks2 && $cubs) {
    ...
    while ($cub = $cubs->fetch_assoc()) {
        ...
        foreach ($task_list as $task_id => $task_title) {
            $grade_q = $conn->query("SELECT id, grade FROM grades3 WHERE task_id = $task_id AND individual_id = {$cub['id']}");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_task'])) {
  ...
  $stmt = $conn->prepare("UPDATE tasks3 SET title=?, description=?, deadline=?, video_link=?, duration_minutes=?, grade_over=? WHERE id=?");
...
$edit_tasks = $conn->query("SELECT * FROM tasks3 WHERE target_group = 'cubs' ORDER BY created_at DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
            ...
            <table class="table table-bordered">
              ...
              <?php foreach ($grades_data as $cub): ?>
                ...
                <?php foreach ($cub['grades'] as $grade): ?>
                  <td><input type="number" class="form-control" name="grades3[<?= $grade['id'] ?>]" value="<?= $grade['grade'] ?>">
                  </td>
                <?php endforeach; ?>
                </tr>
              <?php endforeach; ?>
    ...
    <table class="table table-bordered">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
          <select name="attendance3[<?= $cub['id'] ?>]" class="form-control">
