...
if ($age < 10 || $age > 15) {
    echo "Access Denied. Only cubs (10 - 15 years old) can access this page.";
    exit();
}

// Fetch tasks assigned for cubs
$tasks = [];
$task_query = $conn->query("SELECT * FROM tasks2 WHERE target_group = 'cubs' ORDER BY created_at DESC");
...
// Fetch news
$news_list = [];
$news_query = $conn->query("SELECT * FROM news2 ORDER BY created_at DESC");
...
<?php endif; ?>
        ...
        <div class="tab-pane fade" id="grades" role="tabpanel">
          ...
          <table class="table table-bordered">
            ...
            <tbody>
              <?php
              $grades = $conn->query("SELECT t.title, g.grade, t.grade_over FROM grades2 g JOIN tasks2 t ON g.task_id = t.id WHERE g.individual_id = $userId");
  ...
  <table class="table table-bordered">
    ...
    <tbody>
      <?php
      $attendance = $conn->query("SELECT attend_date, status FROM attendance2 WHERE individual_id = $userId ORDER BY attend_date DESC");
